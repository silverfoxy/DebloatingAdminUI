                    ...
                    <tr>
                      ...
                      <td style="text-align: center;">
                        ...
                        <div class="btn-group csv_btn_group" role="group" aria-label="Action button group" style="display: none">
                          ...
                          <a href="<?php echo site_url('report/delete_testgroup/'.$t['test_group']); ?>" onclick="return confirm('Are you sure you want to delete all the data related to this test group?')" class="btn btn-danger btn-xs"><span class="fa fa-trash"></span> Delete All</a>
                      </td>
                    </tr>
                    <?php } ?>
                  </table>
    ...
    $('input[name="multiselect_chk"]').change(function() {
          ...
          $('input[name="multiselect_chk"]').each(function(){
            ...
            if(checked_items.length === 2) {
                $('#diff_coverage').removeAttr("disabled");
            }
            else {
                $('#diff_coverage').attr('disabled', 'disabled');
            }
          if (checked_items.length <= 0) {
            ...
            update_union_hrefs(output_format, checked_items.length);
          }
      });
      ...
      function update_union_hrefs(output_format, $checked_items_length)
      {
        ...
        base_href =  '<?php echo site_url("report/diff/"); ?>';
        $('#diff_coverage').attr('href', base_href + checked_items.join('/'));
        if ($checked_items_length > 2) {
            $('#diff_coverage').attr('disabled', 'disabled');
        }
      }
});
